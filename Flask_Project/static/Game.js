...
$(document).ready(function(){
    ...
    $("#draw_button").click(function(){
        ...
        if (gameEnd){
            $("#notification_text").html("Game has already ended, to continue please refresh the page")
            return
        ...
        //I should draw based on the current player
            if (!isDraw){
             ...
             //sales=sales,clients=clients, current_id=current_id
                ...
                $("#notification_text").html("Card already drawn, can't draw again until next turn.")
     ...
     $("#turn_button").click(function(){
        ...
        if (coinfection == false){
            ...
            if (current_player==1){
                ...
                current_player=2
                loadhand(playerTwo["hand"])
                $("#notification_text").html("Player 1 Turn End, Player 2 Turn Start. Draw Card!")
                ...
                current_player=1
                loadhand(playerOne["hand"])
                $("#notification_text").html("Player 2 Turn End, Player 1 Turn Start. Draw Card!")
        ...
        // just in case the user used coinfection, i don't want to go to next player's turn
            ...
            if (current_player==2){
                ...
                current_player=2
                loadhand(playerTwo["hand"])
                $("#notification_text").html("Coinfection used, player 2 starts new turn")
                $("#player_text").html("Player 2")
                $("#virus_image").attr("src",playerTwo["virusImage"])
                $("#stage_image").attr("src",playerTwo["stageImage"])
            }
                ...
                current_player=1
                loadhand(playerOne["hand"])
                $("#notification_text").html("Coinfection used, player 1 starts new turn")
                $("#player_text").html("Player 1")
                $("#virus_image").attr("src",playerOne["virusImage"])
                $("#stage_image").attr("src",playerOne["stageImage"])
            }
            ...
            success: function(result){
                ...
                if (current_player==1){
                    ...
                    if(playerOne["stage"]==5){
                        gameEnd=true
                        $("#notification_text").html("GAME END, YOU WON!. Refresh page to play again")
...
function loadhand (hand){
    ...
    for( card in hand){
        ...
        var newRow = $("<img alt='Bootstrap Image Preview' id = '" + card + "' src='" + hand[card]["image"].toString() + "' onclick='playCard(" + card + ")' width='250' height='400' class='d-inline-block align-top' />")
        $("#hand").append(newRow)
        if (hand[card]["name"].endsWith("Rapid Growth")){
            newRow = $("<button type='button' id = '" + card + "' onclick='hold(" + card + ")' class='extraButton'>")
...
function playCard(index){
    ...
    if (playingCard){
        ...
        $("#notification_text").html("Click play button first before clicking card to use it.")
    }
...
function playAntibiotic(index,player,card){
    ...
    $.ajax({
        ...
        success: function(result){
            ...
            loadhand(player["hand"])
            $("#notification_text").html("Antibiotic activated, drawing 2 cards! Drew:  " + result["cards"][0]["name"] + " and " + result["cards"][1]["name"])
        },
        error: function(request, status, error){
            console.log("Error");
            console.log(request)
...
function playAntiviral(index,player,card){
    $("#notification_text").html("Antiviral Activated, other player's hand has been discarded!")
    if (player["player"]==1){
        playerTwo["hand"]=[]
        loadhand(playerOne["hand"])
...
function playCoinfection(index,player,card){
    $("#notification_text").html("Coinfection Activated, draw 1 card and skip other player")
    $.ajax({
        type: "POST",
        url: "/draw",                
        dataType : "json",
    ...
    if (card["immediate"]){
        ...
        $.ajax({
            ...
            success: function(result){
                ...
                $("#notification_text").html("Activating No vaccine Immediate., hold card and drew:" +result["drawnCard"]["name"] )
    ...
    $.ajax({
        ...
        success: function(result){
            if (result["downGraded"]==true){
                ...
                $("#notification_text").html("Quarantine activated, shuffling back into deck!")
            }
            if (current_player==1){
                console.log()
                playerOne=result["player"]
...
function playTreatement(index,player,card,virus){
    ...
    if (current_player==1 && playerTwo["virus"].endsWith(virus)){
        ...
        if (found){
            ...
            $("#notification_text").html("Opponent has Resitance to cancel your Treatement card")

        }
        else{
            //In this case, no resistance was found, so that means I can get player 2's strongest card
            if (playerTwo["hand"].length==0){
                $("#notification_text").html("Oppenent has no cards, wasted your Treatement effect")   
            }
            else{
                ...
                $("#notification_text").html("Stole enemy "+ enemyCard["name"] + " card from oponent")
            }
        }
        ...
        $("#notification_text").html("Wasted your card, no player has virus to match with your treatement")
    }
    loadhand(player["hand"])
    updatePlayer(playerOne,1)
    updatePlayer(playerTwo,2)
