...
def test_AdvanceStageMax():
    player = Player(1,"flu")
    player.stage=5
    assert player.advanceStage(data={"hand":[],"stage":"1","atp":"2", "stageImage":"/testImage"}) == False

def test_AdvanceStageValid():
    player = Player(1,"flu")
    player.advanceStage(data={"hand":[],"stage":"0","atp":"200", "stageImage":"/testImage"})
    assert player.stage == 1
    assert player.stageImage == "/static/images/stage/stage_2.png"
def test_AdvanceStageInvalid():
    player = Player(1,"flu")
    assert player.advanceStage(data={"hand":[],"stage":"0","atp":"0", "stageImage":"/testImage"}) == False

def test_findVirusImageFlu():
    player = Player(1,"flu")
    assert player.findVirusImage(player.virus) == "/static/images/virus/flu.png"

def test_findVirusImageEbola():
    player = Player(1,"ebola")
    assert player.findVirusImage(player.virus) == "/static/images/virus/ebola.png"
...
def test_quarantineValid():
    player = Player(1,"flu")
    player.stage=2
    player.quaranine(data={"hand":[],"stage":"2","atp":"200", "stageImage":"/testImage"})
    assert player.stage == 1
    assert player.stageImage == "/static/images/stage/stage_2.png"